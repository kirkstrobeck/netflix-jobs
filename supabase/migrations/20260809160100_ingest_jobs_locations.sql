-- ingest_jobs, now writing the site join by slug.
--
-- The slugs are resolved in the ingestor, not here: the parser needs the raw
-- string, a country alias table and the curated seed, and none of those belong
-- in plpgsql. The payload therefore carries `location_slugs` alongside the raw
-- `locations` array, and this function only has to fan it out.
--
-- Every slug in that array is already known to be in the seed -- the ingestor
-- upserts public.locations first and drops nothing silently, it reports what it
-- could not place. The foreign key on job_locations.location_slug is the
-- backstop for a bug in that path, and it is deliberately loud: an unknown slug
-- aborts the batch rather than storing a posting with a location that is not
-- in the table.
